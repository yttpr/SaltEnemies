...
namespace Hawthorne
{
    public static class MiniBossUnlockSystem
    {
        ...
        public const string ModID = "SaltHawthorne";//CHANGE THIS
        ...
        public static void Setup()
        {
            ...
            CleanedData = false;
            IDetour TryBeatBossIDetour = (IDetour)new Hook((MethodBase)typeof(UnlockablesManager).GetMethod(nameof(UnlockablesManager.TryBeatBossWith), ~BindingFlags.Default), typeof(MiniBossUnlockSystem).GetMethod(nameof(MiniBossUnlockSystem.TryBeatBossWith), ~BindingFlags.Default));
            IDetour MainStartIDetour = (IDetour)new Hook((MethodBase)typeof(MainMenuController).GetMethod(nameof(MainMenuController.Start), ~BindingFlags.Default), typeof(MiniBossUnlockSystem).GetMethod(nameof(MiniBossUnlockSystem.Start), ~BindingFlags.Default));
            IDetour OverworldIDetour = new Hook(typeof(OverworldManagerBG).GetMethod(nameof(OverworldManagerBG.Awake), ~BindingFlags.Default), typeof(MiniBossUnlockSystem).GetMethod(nameof(StartWorld), ~BindingFlags.Default));
            //IDetour CharaStarIDetour = (IDetour)new Hook((MethodBase)typeof(SelectableCharactersSO).GetMethod(nameof(SelectableCharactersSO.PrepareCharacters), ~BindingFlags.Default), typeof(MiniBossUnlockSystem).GetMethod(nameof(MiniBossUnlockSystem.PrepareCharacters), ~BindingFlags.Default));
        ...
        public static string GetItemSystemName(BrutalAPI.Item w)
        {
            ...
            return "ITEM" + RemoveSpecialChars(wName);
        }
        ...
        public static void StartWorld(Action<OverworldManagerBG> orig, OverworldManagerBG self)
        {
            orig(self);
            Unlocks = self._informationHolder.UnlockableManager;
        }
    ...
    public static class AchievementSystem
    {
        ...
        public static void TryInitializeUnlockableAchievements(Action<UnlockListUIPanel, int, IUnlockCalls, Sprite[]> orig, UnlockListUIPanel self, int listID, IUnlockCalls calls, Sprite[] achInfo)
        {
            ...
            //LayoutRebuilder.ForceRebuildLayoutImmediate(self.GetComponent<RectTransform>());
    ...
    public class UnlockItem : EffectItem
    {
        public bool Added = false;

        public bool Hide = false;

        public static bool gunPrereq = false;
        ...
        public void Prepare()
        {
            ...
            if (MiniBossUnlockSystem.SaveConfigNames == null)
            {
                MiniBossUnlockSystem.SaveConfigNames = new Dictionary<string, bool>();
            }
            string l = MiniBossUnlockSystem.SaveName;
            FileStream inStream = File.Open(MiniBossUnlockSystem.SaveName, FileMode.Open);
            XmlDocument xmlDocument = new XmlDocument();
            xmlDocument.Load((Stream)inStream);
            string kug = MiniBossUnlockSystem.GetItemSystemName(this);
            bool add = false;
            if (xmlDocument.GetElementsByTagName("config").Count > 0)
            {
                if (xmlDocument.GetElementsByTagName("config")[0].Attributes[kug] != null)
                {
                    add = bool.Parse(xmlDocument.GetElementsByTagName("config")[0].Attributes[kug].Value);
                    if (add || Gun && gunPrereq)
                    {
                        AddItem();
                        if (Ach != null) Ach.SetValue(true);
                        if (DoDebugs.GenInfo) Debug.Log("Added: " + this.name);
                        Added = true;
                        if (firstBullet) gunPrereq = true;
                    }
                    else
                    {
                        if (DoDebugs.GenInfo) Debug.Log("Prepared: " + this.name);
                    }
                }
                MiniBossUnlockSystem.SaveConfigNames.Add(kug, add);
            }
            inStream.Close();
        }

        public BaseWearableSO unlockIt = null;
        
        public override BaseWearableSO Wearable()
        {
            BaseWearableSO jogson = base.Wearable();
            unlockIt = jogson;
            return jogson;
        }
        public void GetItem()
        {
            ...
            if (unlockIt != null)
            {
                string str = MiniBossUnlockSystem.GetItemSystemName(this);
                if (MiniBossUnlockSystem.SaveConfigNames.TryGetValue(str, out var unlocked) && unlocked)
                {
                    return;
                }
                try
                {
                    if (!Hide) MiniBossUnlockSystem.Unlocks._freshlyAcquiredItems.Add(MiniBossUnlockSystem.GetItemName(this));
                    if (!Hide) SaveManager.SaveFreshItemsSaveData(MiniBossUnlockSystem.Unlocks._freshlyAcquiredItems);
                }
                catch (Exception ex)
                {
                    if (DoDebugs.MiscInfo)
                    {
                        Debug.LogError("unlock board fucked up");
                        Debug.LogError(ex + ex.StackTrace);
                    }
                }
                MiniBossUnlockSystem.SaveConfigNames[str] = true;
                MiniBossUnlockSystem.WriteConfig(MiniBossUnlockSystem.SaveName);
                unlockIt = null;
                if (DoDebugs.GenInfo) Debug.Log("unlock board: " + this.name);
            }
        }
    }
    public class DoubleEffectUnlockItem : DoubleEffectItem
    {
        public bool Added = false;
        ...
        public void GetItem()
        {
            ...
            if (unlockIt != null)
            {
                ...
                MiniBossUnlockSystem.Unlocks._freshlyAcquiredItems.Add(MiniBossUnlockSystem.GetItemName(this));
                SaveManager.SaveFreshItemsSaveData(MiniBossUnlockSystem.Unlocks._freshlyAcquiredItems);
                MiniBossUnlockSystem.SaveConfigNames[str] = true;
                MiniBossUnlockSystem.WriteConfig(MiniBossUnlockSystem.SaveName);
                unlockIt = null;
            }
