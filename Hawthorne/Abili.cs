...
namespace Hawthorne
{
        ...
        public static Ability Throttle
        {
            get
            {
                if (_throttle == null)
                {
                    ...
                    {
                        name = "Throttle",
                        description = "Deal a Deadly amount of damage to the Opposing party member and heals them a moderate amount of health. If this attack kills, it doesn't, applies Focused and Cursed on them.",
                        rarity = 5,
                        effects = new Effect[]
                        {
                ...
                if (_rightKill == null)
                {
                    _rightKill = new Ability()
                    {
                        ...
                        effects = new Effect[]
                        {
                            ...
                            new Effect(BasicEffects.GetVisuals("Domination_A", false, Slots.Front), 1, null, Slots.Self),
                ...
                if (_leftKill == null)
                {
                    _leftKill = new Ability()
                    {
                        ...
                        effects = new Effect[]
                        {
                            ...
                            new Effect(BasicEffects.GetVisuals("Talons_A", false, Slots.Front), 1, null, Slots.Self),
                            new Effect(ScriptableObject.CreateInstance<DirectDeathEffect>(), 1, IntentType.Damage_Death, Slots.Front),
                        },
                        visuals = null,
                        animationTarget = Slots.Self,
            ...
            get
            {
                if (_seeDreams == null)
                {
                    _seeDreams = new Ability()
                    {
                        ...
                        description = "Inflict 2 Ruptured and apply 3 Dodge on the Opposing party member. Move this enemy to th left or right.",
                        rarity = 4,
                        effects = new Effect[]
                        {
                ...
                if (_dieWithYou == null)
                {
                    _dieWithYou = new Ability()
                    {
                        ...
                        effects = new Effect[]
                        {
                            ...
                            {
                                new Effect(BasicEffects.GetVisuals("Wriggle_A", false, Slots.Front), 1, null, Slots.Self),
            ...
            get
            {
                if (_engravings == null)
                {
                    _engravings = new Ability()
                    {
                        ...
                        visuals = LoadedAssetsHandler.GetCharacterAbility("Shank_1_A").visuals,
                        animationTarget = Slots.Front,
                    };
                }
                return _engravings;
            ...
            get
            {
                if (_longSlice == null)
                {
                    _longSlice = new Ability()
                    {
                        ...
                        effects = new Effect[]
                        {
                            new Effect(BasicEffects.GetVisuals("FlayTheSkin_A", false, Slots.SlotTarget(new int[] {-3, 3}, false)), 1, null, Slots.SlotTarget(new int[] {-3, 3 }, false), HasConfusionCondition.Create(true)),
            ...
            get
            {
                if (_librarium == null)
                {
                    _librarium = new Ability()
                    {
                        ...
                        effects = new Effect[]
                        {
                            new Effect(BasicEffects.GetVisuals("Wriggle_A", false, NoiseTargetting.Default()), 1, null, NoiseTargetting.Default()),
                            new Effect(BasicEffects.Die(true), 1, IntentType.Damage_Death, NoiseTargetting.Default()),
                            new Effect(BasicEffects.GetVisuals("Wriggle_A", false, Slots.SlotTarget(new int[] {-2, 0, 2 }, false)), 1, null, NoiseTargetting.Default(), BasicEffects.DidThat(true)),
    ...
    public class PlaySoundEffect : EffectSO
    {
        ...
        public override bool PerformEffect(CombatStats stats, IUnit caster, TargetSlotInfo[] targets, bool areTargetSlots, int entryVariable, out int exitAmount)
        {
            ...
            RuntimeManager.PlayOneShot(Audio, default(Vector3));
                ...
                if (self._enemiesInCombat.TryGetValue(enemyID, out var value))
                {
                    ...
                    if (!(abilityBySlotID == null) && !abilityBySlotID.Equals(null))
                    {
                        ...
                        if (abilityBySlotID._abilityName == Abili.Patience.name)
                        {
                            StringPairData abilityLocData = abilityBySlotID.GetAbilityLocData();
                            header = abilityLocData.text;
                            content = "Move all party members with Terror closer to this enemy. If there are no party members with Terror, apply it to a random party member.";
                            self._tooltip.DelayShow(content, header, "");
                            return;
                        }
                    }
    ...
    public class EngravingsEffect : EffectSO
    {
        ...
        public override bool PerformEffect(CombatStats stats, IUnit caster, TargetSlotInfo[] targets, bool areTargetSlots, int entryVariable, out int exitAmount)
        {
            ...
            if (Shank == null) Shank = BasicEffects.GetVisuals("Shank_1_A", true, Slots.Sides);
